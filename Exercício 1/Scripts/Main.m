...
est.N = 100;                %Number of measurements
...
ls.thetaLS = zeros(2,est.N);    %Least Squares estimated parameter
...
map.thetaMAP = zeros(2,est.N);  %Maximum a Posteriori estimated parameter
map.RMS_MAP = zeros(2,2);       %Maximum a Posteriori RMS Error

%% Simulation

for n = 1:1000                                          %Estimation Loop
    est.theta = sqrtm(est.P_t)*randn(2,1) + est.m_t;    %Parameter values definition
    for m = 1:est.N                                     %Monte Carlo simulation loop
        v = est.R*randn(1);                             %Measurement Noise 
        est.y(m,1) = est.H(m,:)*est.theta + v;          %Measurement Equation
    end
